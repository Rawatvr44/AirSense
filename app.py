...
class DrawInAir:
    ...
    def streamlit_config(self):
        ...
        st.markdown(f'<h1 style="text-align: center; width:70%; border-radius:20px; background-color:BlanchedAlmond;">AirSense</h1>', unsafe_allow_html=True)
    ...
    def main(self):
        ...
        with col3:
            ...
            st.markdown("""
                <h5 style="text-align: center; border-radius:20px; background-color:BlanchedAlmond; color :red;">Finger Gestures:</h5>
                <ul background-color:BlanchedAlmond; color :cyan;>
            ...
            st.markdown(f'<h5 style="text-position:center; text-align: center;  border-radius:20px;       color:red;background-color:BlanchedAlmond; text-size:10px;">OUTPUT:</h5>', unsafe_allow_html=True)
        ...
        while True:
            ...
            if sum(self.fingers) == 2 and self.fingers[1] == self.fingers[2] == 1:
                ...
                result_placeholder.markdown(f'<h2 style="text-align: center; font-size: 20px;">Result: {result}</h2>', unsafe_allow_html=True)
...
def main_app():
    st.title("GenerativeAI-Powered Applications -By Team Rawatvr44")
    app_mode = st.selectbox("Choose an application:", ["AirSense", "Image Reader", "Plot Crafter"])
    
    if app_mode == "AirSense":
        drawinair = DrawInAir()
